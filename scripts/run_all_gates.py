...
GATES = [
    ...
    ("G398", "**落盘清单与台账同源**（哪些文件不对须离线可答）",
     # 🔴 第一百零三轮诚实结论①：完整清单只在远端
     #    → 远端不可达时"哪些文件不对"**又回答不了**。
     # 🔑 G398 查**本地落盘**：① 不缺清单 ② 无僵尸
     #    ③ 行数与指纹与台账同源。
     # 🔑 与 G397 不可互相替代：G397 查远端防台账撒谎，
    ...
    # 🔑 第一百零七轮：**基准指纹**必须未被改过
    #    🔴 第一百零六轮诚实结论⑥：某文件 chmod +x → 基准松了 →
    #       历史遗留**集体消失**，而没有任何东西解释"为什么突然没了"。
    #       🔑 "消失"与"被修复"必须能被区分。
    ("G400", "**基准指纹未被改过**（防遗留“消失”被误读成“修复”）",
     ["{py}", "{s}/push_api.py", "--assert-baseline-fp"], "hard", []),
    # 🔑 第一百零八轮：**变更史**须闭合且可答"哪一轮变的、为什么变"
    #    🔴 第一百零七轮诚实结论②③：只留最新指纹 →
    #       "什么时候变的、为什么变"无从回答；
    #       "没变更过"与"变更史被删"无法区分。
    ("G401", "**基准变更史闭合**（可答“哪一轮变的、为什么变”）",
     ["{py}", "{s}/push_api.py", "--assert-fp-history"], "hard", []),
    # 🔑 第一百零九轮：**镜像**必须与台账一致 —— 防"整段替换变更史"
    #    🔴 108 轮②实测：G401 只验链条闭合 → 重造一段自洽的照样 rc=0。
    #    🔑 镜像放在 ledger/（与台账 audit/ 不同目录）→ 须同时改两处。
    ("G402", "**变更史镜像与台账一致**（防整段替换变更史）",
     ["{py}", "{s}/push_api.py", "--assert-fp-mirror"], "hard", []),
]
...
BOTH_LAYERS = (
    ('G397', 'G398',
     ...
     '只留 G397 → 断网时拒绝给结论。'),
    # 🔑 第一百零六轮：补上第二组（第一百零五轮诚实结论①）
    ('G396', 'G398',
     'G396 查远端断言"遗留集合未变"（历史被改写会立刻暴露）；'
     'G398 查落盘断言"清单可离线回答"。'
     '删任一个都开洞：只留 G398 → 历史被 force push 改写时'
     '落盘与台账仍同源却**一起错**；只留 G396 → 断网时拒绝给结论。'),
    # 🔑 第一百零九轮：补上第三组（第一百零八轮诚实结论②）
    ('G401', 'G402',
     'G401 验变更史**自身**闭合（形状 + 链条 + seq）；'
     'G402 验变更史与**另一处镜像**一致（防整段替换）。'
     '🔴 实测证明必须并存：整段重造一段自洽变更史时 G401 **rc=0**'
     '（链条闭合、seq 连续、末条指纹相符），只有 G402 抓到。'),
)
