...
| 🔑 第一百零五轮：落盘改 `mode\tpath` + G399 **双层并存断言**；🔑 判据"命令必须互异"防同一实现挂两个号；🔴 一次破坏没做成险些记成"兼容通过" | `references/游戏引擎复刻.md` 第一百零五部分 |
| 🔑 第一百零六轮：(G396,G398) 第二组登记 + 落盘三列；🔴 修**真实漏网**：台账缺 `baseline_modes` 时判据静默跳过 → 破坏 rc=0；🔑 "应有 mode"改为逐文件取自索引 | `references/游戏引擎复刻.md` 第一百零六部分 |
| 🔑 第一百零七轮：新增 **G400 基准指纹断言**；🔴 解决"某文件 chmod +x → 基准松了 → 历史遗留集体消失"却无人解释"为什么突然没了"；🔑 遗留归零不等于修好了 | `references/游戏引擎复刻.md` 第一百零七部分 |
| 🔑 第一百零八轮：新增 **G401 基准变更史闭合**；🔴 实测抓到解析 bug：`一百零七` 被算成 **100** 且**不报错**；🔑 链条闭合判据（每条 from_fp == 上一条 to_fp）防"中间变更被删" | `references/游戏引擎复刻.md` 第一百零八部分 |
| 🔑 第一百零九轮：新增 **G402 变更史镜像**；🔴 实测证明 G401 对"重造的自洽变更史"**rc=0**（108 轮②是真的）；🔑 镜像放**指纹**不放副本（副本形成同源自证）；⚠️ 同时改两处仍可通过——已知边界 | `references/游戏引擎复刻.md` 第一百零九部分 |
